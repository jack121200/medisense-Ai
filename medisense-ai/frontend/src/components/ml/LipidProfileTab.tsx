...
// ── Colour tokens ──────────────────────────────────────────────────────────────
const C = {
    lipid:    '#00B4D8',   // teal-blue for lipid
    gold:     '#FFD166',
    crimson:  '#E63946',
    teal:     '#06D6A0',
    purple:   '#C77DFF',
...
function Field({ label, value, onChange, placeholder = '', unit = '' }: {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.07em', display: 'flex', justifyContent: 'space-between' }}>
                <span>{label}</span>
                {unit && <span style={{ color: 'rgba(255,255,255,0.25)', fontWeight: 400 }}>{unit}</span>}
            </label>
            <input
                ...
                style={{
                    padding: '9px 12px',
                    background: focused ? 'rgba(0,180,216,0.06)' : 'rgba(255,255,255,0.04)',
                    border: `1px solid ${focused ? `${C.lipid}60` : 'rgba(255,255,255,0.09)'}`,
...
function Toggle({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
    return (
        <button onClick={onClick} style={{
            ...
            background: active ? `${C.lipid}18` : 'rgba(255,255,255,0.03)',
            border: `1px solid ${active ? `${C.lipid}50` : 'rgba(255,255,255,0.07)'}`,
            color: active ? C.lipid : 'rgba(255,255,255,0.45)',
            cursor: 'pointer', fontSize: 13, fontWeight: active ? 700 : 500,
            transition: 'all 0.2s',
        }}>
            <div style={{
                width: 16, height: 16, borderRadius: '50%',
                background: active ? C.lipid : 'rgba(255,255,255,0.15)',
...
export default function LipidProfileTab() {
    ...
    const handlePdfUpload = async (file: File) => {
        ...
        try {
            ...
            fd.append('file', file);
            const res = await axios.post('http://localhost:8000/api/pdf/extract-lipid', fd);
    ...
    const PanelRow = ({ paramKey, p }: { paramKey: string; p: any }) => {
        const col = STATUS_COLOR[p.status] || '#fff';
        ...
        return (
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}
                ...
                <td style={{ padding: '11px 16px', fontWeight: 700, color: '#fff', fontSize: 13 }}>{labels[paramKey] || paramKey}</td>
                <td style={{ padding: '11px 16px', fontFamily: 'monospace', color: col, fontWeight: 800, fontSize: 14 }}>{p.value}</td>
                <td style={{ padding: '11px 16px', color: 'rgba(255,255,255,0.35)', fontSize: 12 }}>{p.unit}</td>
                <td style={{ padding: '11px 16px', color: 'rgba(255,255,255,0.35)', fontSize: 11, maxWidth: 200 }}>{p.reference}</td>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    ...
                    <div style={{ background: risk.bg, border: `2px solid ${risk.color}35`, borderRadius: 18, padding: '24px 20px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Cardiovascular Risk</div>
                        ...
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>probability</div>
                    </div>
                    ...
                    <div style={{ background: statin.bg, border: `2px solid ${statin.color}35`, borderRadius: 18, padding: '24px 20px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Statin Therapy</div>
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 14, padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>NLA-2014 LDL Category</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: '#fff' }}>{d.nla_category}</div>
                    </div>
                    <Info size={18} color="rgba(255,255,255,0.2)" />
                </div>
                ...
                {(d.has_dyslipidemia || d.has_metabolic_syndrome) && (
                    <div style={{ background: `${C.amber}08`, border: `1px solid ${C.amber}30`, borderRadius: 14, padding: 16 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Detected Conditions</div>
                ...
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>Key Cardiovascular Ratios</div>
                        ...
                        {Object.entries(d.ratios || {}).map(([k, v]: [string, any]) => {
                            ...
                            return (
                                <div key={k} style={{ background: `${col}10`, border: `1px solid ${col}25`, borderRadius: 10, padding: '10px 14px' }}>
                                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>{ratioLabels[k] || k}</div>
                ...
                {d.red_flags?.length > 0 && (
                    <div style={{ background: `${C.crimson}10`, border: `1px solid ${C.crimson}30`, borderRadius: 14, padding: 16 }}>
                        ...
                        {d.red_flags.map((f: string, i: number) => (
                            <div key={i} style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.75)', lineHeight: 1.65, marginBottom: 6 }}>• {f}</div>
                        ))}
                    </div>
                )}
                ...
                {/* ── Clinical Message ── */}
                <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Clinical Summary</div>
                    <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.72)', lineHeight: 1.75 }}>{d.clinical_message}</div>
                </div>
    ...
    const PanelTable = ({ panel }: { panel: any }) => (
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 18, overflow: 'hidden', marginBottom: 20 }}>
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ fontWeight: 800, fontSize: 15, color: '#fff' }}>Lipid Panel — NLA-2014 Reference</div>
            </div>
            ...
            <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 18, padding: 28, marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                            <div style={{ fontWeight: 800, fontSize: 16, color: '#fff' }}>Lipid Profile Analyzer</div>
                            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>Random Forest · NLA-2014 · Cardiovascular Risk</div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 6, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 4 }}>
                        {(['manual', 'pdf'] as const).map(m => (
                            <button key={m} onClick={() => setInputMode(m)} style={{
                                ...
                                color: inputMode === m ? C.lipid : 'rgba(255,255,255,0.4)',
                ...
                {inputMode === 'pdf' ? (
                    <div style={{ border: `2px dashed ${C.lipid}30`, borderRadius: 14, padding: 44, textAlign: 'center', background: `${C.lipid}04` }}>
                        ...
                        <div style={{ fontSize: 15, color: '#fff', fontWeight: 800, marginBottom: 6 }}>Upload Lipid Panel Report</div>
                        <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.4)', marginBottom: 6, lineHeight: 1.7 }}>
                            Supports <strong style={{ color: 'rgba(255,255,255,0.65)' }}>PDF</strong> and <strong style={{ color: 'rgba(255,255,255,0.65)' }}>DOCX</strong>.
                            <br />AI extracts Total Cholesterol, LDL, HDL, VLDL, Triglycerides, Non-HDL automatically.
                        </div>
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginBottom: 24 }}>You can review and edit all values before running the analysis.</div>
                    ...
                    <>
                        {/* ── Lipid Panel Values ── */}
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>Lipid Panel Values</div>
                        ...
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>Patient Demographics</div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 14, marginBottom: 20 }}>
                            ...
                            {/* Gender selector */}
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                                <label style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Gender</label>
                                <select value={form.gender} onChange={e => setForm(f => ({ ...f, gender: e.target.value }))}
                                    style={{ padding: '9px 12px', background: '#1a1a2a', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 9, color: '#fff', fontSize: 13.5, outline: 'none', fontFamily: 'inherit', cursor: 'pointer' }}>
                        ...
                        {/* ── Risk Flags ── */}
                        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Risk Factors</div>
                        ...
                        <button onClick={submit} disabled={loading} style={{
                            ...
                            background: loading ? 'rgba(255,255,255,0.08)' : `linear-gradient(135deg, ${C.lipid}, #0077B6)`,
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 22, alignItems: 'start' }}>
                    ...
                    <div>
                        ...
                        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20 }}>
                            <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>Risk Probability Distribution</div>
