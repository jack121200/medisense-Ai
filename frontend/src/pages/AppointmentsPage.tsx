...
function ApproveModal({ reqId, defaultDate, onClose, onDone }: { reqId: string; defaultDate: string; onClose: () => void; onDone: () => void }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(90, 65, 45, 0.16)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(62, 142, 126, 0.25)', borderRadius: 20, padding: 32, width: 440 }}>
...
function RejectModal({ reqId, onClose, onDone }: { reqId: string; onClose: () => void; onDone: () => void }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(90, 65, 45, 0.16)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(204, 107, 61, 0.25)', borderRadius: 20, padding: 32, width: 460 }}>
                ...
                <div style={{ padding: '14px 16px', background: 'rgba(204, 107, 61, 0.06)', border: '1px solid rgba(204, 107, 61, 0.15)', borderRadius: 12, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
                {FILTERS.map(f => (
                    <button key={f} onClick={() => setFilter(f)} style={{
                        ...
                        background: filter === f ? 'rgba(194, 91, 60, 0.12)' : 'var(--surface-2)',
                        ...
                        <div key={req.id} style={{ padding: '18px 22px', borderBottom: '1px solid var(--surface-border)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(194, 91, 60, 0.12)', border: '1px solid rgba(194, 91, 60, 0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>👤</div>
                                    <div>
                            ...
                            {req.status === 'APPROVED' && req.scheduledDate && (
                                <div style={{ padding: '8px 12px', background: 'rgba(62, 142, 126, 0.06)', border: '1px solid rgba(62, 142, 126, 0.15)', borderRadius: 8, fontSize: 12.5, color: 'var(--risk-low)', marginTop: 8 }}>
                            ...
                            {req.status === 'PENDING' && (
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <button onClick={() => { setApproving(req.id); setApprovingReq(req); }}
                                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', background: 'rgba(62, 142, 126, 0.08)', border: '1px solid rgba(62, 142, 126, 0.2)', borderRadius: 8, color: 'var(--risk-low)', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                                        <CheckCircle size={12} /> Approve & Schedule
                                    </button>
                                    <button onClick={() => setRejecting(req.id)}
                                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', background: 'rgba(204, 107, 61, 0.08)', border: '1px solid rgba(204, 107, 61, 0.2)', borderRadius: 8, color: 'var(--risk-high)', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
...
function BookTab() {
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, alignItems: 'flex-start' }}>
            {/* Form */}
            <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(194, 91, 60, 0.18)', borderRadius: 18, padding: '24px 26px', position: 'relative', overflow: 'hidden' }}>
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', background: 'rgba(62, 142, 126, 0.04)', border: '1px solid rgba(62, 142, 126, 0.12)', borderRadius: 9, fontSize: 12, color: 'var(--text-secondary)' }}>
            ...
            <div>
                ...
                {booked.length === 0 ? (
                    ...
                    <div key={b.id} style={{ background: 'var(--surface-1)', border: '1px solid rgba(194, 91, 60, 0.14)', borderRadius: 14, padding: '14px 18px', marginBottom: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 20, color: 'var(--risk-low)', background: 'rgba(62, 142, 126, 0.1)', border: '1px solid rgba(62, 142, 126, 0.2)' }}>BOOKED</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
                {tabs.map(t => (
                    <button key={t.id} onClick={() => setActiveTab(t.id as any)} style={{
                        ...
                        border: `1px solid ${activeTab === t.id ? 'rgba(194, 91, 60, 0.3)' : 'var(--surface-border)'}`,
                        background: activeTab === t.id ? 'rgba(194, 91, 60, 0.06)' : 'var(--surface-1)',
