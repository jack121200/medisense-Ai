...
function ApproveModal({ reqId, defaultDate, onClose, onDone }: { reqId: string; defaultDate: string; onClose: () => void; onDone: () => void }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(0,255,135,0.25)', borderRadius: 20, padding: 32, width: 440 }}>
...
function RejectModal({ reqId, onClose, onDone }: { reqId: string; onClose: () => void; onDone: () => void }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(255,107,53,0.25)', borderRadius: 20, padding: 32, width: 460 }}>
                ...
                <div style={{ padding: '14px 16px', background: 'rgba(255,107,53,0.06)', border: '1px solid rgba(255,107,53,0.15)', borderRadius: 12, marginBottom: 20 }}>
                    <div style={{ fontSize: 11.5, fontWeight: 700, color: '#FF6B35', marginBottom: 10 }}>↩ Suggest Alternative (optional)</div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button onClick={onClose} className="btn-ghost" style={{ flex: 1 }}>Cancel</button>
                    <button onClick={handle} disabled={saving} style={{ flex: 2, padding: '10px 0', background: 'linear-gradient(135deg, #FF6B35, #c0392b)', border: 'none', borderRadius: 10, color: '#fff', fontWeight: 800, cursor: 'pointer', fontSize: 14 }}>
...
function RequestsTab() {
    ...
    const stats = [
        ...
        { label: 'Pending', val: pendingCount, color: '#FFD166' },
        ...
        <div>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
                {FILTERS.map(f => (
                    <button key={f} onClick={() => setFilter(f)} style={{
                        ...
                        background: filter === f ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${filter === f ? '#6366f140' : 'rgba(255,255,255,0.08)'}`,
            ...
            <div style={{ background: 'var(--surface-1)', border: '1px solid var(--surface-border)', borderRadius: 18, overflow: 'hidden' }}>
                {loading ? (
                    ...
                    return (
                        <div key={req.id} style={{ padding: '18px 22px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>👤</div>
                                    <div>
                                        ...
                                        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
                                            {docName} · <span style={{ color: '#6366f1' }}>{req.doctor?.specialization}</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {req.status === 'APPROVED' && req.scheduledDate && (
                                <div style={{ padding: '8px 12px', background: 'rgba(0,255,135,0.06)', border: '1px solid rgba(0,255,135,0.15)', borderRadius: 8, fontSize: 12.5, color: '#00FF87', marginTop: 8 }}>
                            ...
                            {req.status === 'PENDING' && (
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <button onClick={() => { setApproving(req.id); setApprovingReq(req); }}
                                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', background: 'rgba(0,255,135,0.08)', border: '1px solid rgba(0,255,135,0.2)', borderRadius: 8, color: '#00FF87', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                                        <CheckCircle size={12} /> Approve & Schedule
                                    </button>
                                    <button onClick={() => setRejecting(req.id)}
                                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', background: 'rgba(255,107,53,0.08)', border: '1px solid rgba(255,107,53,0.2)', borderRadius: 8, color: '#FF6B35', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
...
function BookTab() {
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, alignItems: 'flex-start' }}>
            {/* Form */}
            <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(0,229,255,0.18)', borderRadius: 18, padding: '24px 26px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div>
                ...
                {booked.length === 0 ? (
                    ...
                    <div key={b.id} style={{ background: 'var(--surface-1)', border: '1px solid rgba(0,229,255,0.14)', borderRadius: 14, padding: '14px 18px', marginBottom: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 20, color: '#00FF87', background: 'rgba(0,255,135,0.1)', border: '1px solid rgba(0,255,135,0.2)' }}>BOOKED</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
                {tabs.map(t => (
                    <button key={t.id} onClick={() => setActiveTab(t.id as any)} style={{
                        ...
                        border: `1px solid ${activeTab === t.id ? 'rgba(0,229,255,0.3)' : 'var(--surface-border)'}`,
                        background: activeTab === t.id ? 'rgba(0,229,255,0.06)' : 'var(--surface-1)',
