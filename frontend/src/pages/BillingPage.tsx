    ...
    return (
        <div style={{ padding: '32px 40px', maxWidth: 1100, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 28 }}>
                <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(63, 138, 102, 0.2)', borderRadius: 16, padding: '20px 22px' }}>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>TODAY'S REVENUE</div>
                    <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--risk-low-text)', fontFamily: 'var(--font-mono)' }}>₹{revenue.revenue.toLocaleString()}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{revenue.paidCount} payments today</div>
                </div>
                <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(201, 154, 42, 0.2)', borderRadius: 16, padding: '20px 22px' }}>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>PENDING COLLECTION</div>
                    <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--risk-medium-text)', fontFamily: 'var(--font-mono)' }}>₹{unpaidTotal.toLocaleString()}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{invoices.filter((i: any) => !i.isPaid).length} unpaid bills</div>
                </div>
                <div style={{ background: 'var(--surface-1)', border: '1px solid rgba(35, 83, 71, 0.2)', borderRadius: 16, padding: '20px 22px' }}>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>TOTAL INVOICES</div>
                    <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--accent-primary)', fontFamily: 'var(--font-mono)' }}>{invoices.length}</div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                {(['all', 'unpaid', 'paid'] as const).map(f => (
                    <button key={f} onClick={() => setFilter(f)} style={{ padding: '7px 18px', borderRadius: 20, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: filter === f ? 'rgba(63, 138, 102, 0.15)' : 'var(--surface-2)', border: `1px solid ${filter === f ? 'rgba(63, 138, 102, 0.3)' : 'var(--surface-border)'}`, color: filter === f ? 'var(--risk-low-text)' : 'var(--text-secondary)', textTransform: 'capitalize' }}>{f}</button>
                ))}
            </div>
                        ...
                        : filtered.map((inv: any) => (
                            <div key={inv.id} style={{ padding: '16px 24px', borderBottom: '1px solid var(--surface-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{ fontSize: 18, fontWeight: 900, color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>₹{inv.totalAmount?.toLocaleString()}</span>
                                    {inv.isPaid
                                        ? <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--risk-low-text)', background: 'var(--risk-low-bg)', padding: '4px 12px', borderRadius: 20, border: '1px solid rgba(63, 138, 102, 0.3)', display: 'flex', alignItems: 'center', gap: 4 }}><CheckCircle size={11} /> PAID</span>
                                        : <button onClick={() => { setSelected(inv); setPayMethod('CASH'); }} style={{ padding: '8px 18px', background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-magenta))', border: 'none', borderRadius: 10, color: 'var(--bg-primary)', fontWeight: 800, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}><CreditCard size={12} /> Collect Payment</button>
                                    }
                                </div>
                            </div>
                        ))}
            ...
            {selected && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(5, 31, 32, 0.16)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div style={{ background: 'var(--surface-2)', border: '1px solid var(--surface-border-md)', borderRadius: 20, padding: 32, width: 'min(420px, calc(100vw - 32px))' }}>
                        ...
                        <div style={{ background: 'rgba(35, 83, 71, 0.06)', border: '1px solid rgba(35, 83, 71, 0.15)', borderRadius: 12, padding: 16, marginBottom: 20, textAlign: 'center' }}>
                        ...
                        <div style={{ marginBottom: 20 }}>
                            ...
                            <div style={{ display: 'flex', gap: 10 }}>
                                {['CASH', 'UPI', 'CARD'].map(m => (
                                    <button key={m} onClick={() => setPayMethod(m)} style={{ flex: 1, padding: '12px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', background: payMethod === m ? 'rgba(35, 83, 71, 0.15)' : 'var(--surface-2)', border: `1px solid ${payMethod === m ? 'rgba(35, 83, 71, 0.4)' : 'var(--surface-border-md)'}`, color: payMethod === m ? 'var(--accent-primary)' : 'var(--text-secondary)' }}>{m}</button>
                                ))}
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: 10 }}>
                            ...
                            <button onClick={markPaid} disabled={paying} style={{ flex: 2, padding: 12, background: 'var(--accent-green-dim)', border: 'none', borderRadius: 10, color: '#fff', fontWeight: 800, cursor: 'pointer' }}>{paying ? 'Processing...' : '✓ Confirm Payment'}</button>
                        </div>
                    </div>
                </div>
            )}
